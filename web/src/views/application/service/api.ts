...
export const appApi = {
  // 获取应用列表
  getAppList: (params?: { app_type?: number | null; test?: number | null }) => {
    return http.get<ApiResponse>(`/appList/getAppList?r=${new Date().getTime()}`, { params })
  },
  // 获取应用分组列表
  ...
  // 添加应用
  addApp: (params?: any) => {
    return http.post<ApiResponse>(`/appList/addApp`, { ...params })
  },
  // 编辑应用
  editApp: (params?: any) => {
    return http.post<ApiResponse>(`/appList/updateApp`, { ...params })
  },
  // 删除应用
  removeApp: (params?: any) => {
    return http.post<ApiResponse>(`/appList/removeApp`, { ...params })
  },
}
