<template>
  ...
  >
    ...
    <input
      ...
      :disabled="animating"
      type="text"
      class="input-container relative z-50 size-full rounded-full border-none bg-transparent pl-4 pr-20 text-sm text-black focus:outline-none focus:ring-0 sm:pl-10 sm:text-base dark:text-white"
      :class="{ 'text-transparent dark:text-transparent': animating }"
      @keydown.enter="handleKeyDown"
      @focus="showTransition = false"
      @blur="handleBlur"
    />
    ...
    <button
      ...
      type="submit"
      class="search-btn absolute right-2 top-1/2 z-50 flex size-8 -translate-y-1/2 items-center justify-center rounded-full bg-black transition duration-200 disabled:bg-gray-100 dark:bg-zinc-900 dark:disabled:bg-zinc-700"
    >
      <svg
    ...
    <div v-if="loading" class="btn-loading-container">
      <div class="btn-loading c-flex-x-center">
    ...
    animationFrame.value = requestAnimationFrame(() => {
      ...
      for (const current of newDataRef.value) {
        if (current.x < start) {
          ...
          current.x += Math.random() > 0.5 ? 1 : -1.5
