<template>
  <div
    ref="containerRef"
    :class="[
      'cpt-carousel relative overflow-hidden',
      round ? 'rounded-full border border-[#333]' : 'rounded-[17px] border border-[#333]',
    ]"
    :style="{
  ...
  >
    ...
    >
      <Motion
        ...
        :class="[
          'carousel-item',
          'relative shrink-0 flex flex-col overflow-hidden cursor-pointer active:cursor-grabbing',
          round
        ...
        :transition="effectiveTransition"
        @mousedown="() => itemMouseDown(item, index)"
        @mouseup="() => itemMouseUp(item, index)"
      >
        <div :class="[`${round} ? 'p-0 m-0' : 'mb-4 p-5'`, 'bg-icon']">
          <span
            v-if="item.icon"
        ...
        <div class="bg-part p-5" :style="{ backgroundImage: `url(${item.bgImage})` }">
          <div class="bg-part-inner absolute left-0 bottom-0">
    ...
    >
      <div :class="['mt-4 flex justify-between px-8', items.length > 1 ? 'w-[150px]' : '']">
        <Motion
...
<script setup lang="ts">
  ...
  const setCurrentIndex = (item: CarouselItem, index: number) => {
    console.log(item)
    currentIndex.value = index
  }

  let timeClickRecord = 0
  const itemMouseDown = (item: CarouselItem, index: number) => {
    timeClickRecord = Date.now()
  }

  const itemMouseUp = (item: CarouselItem, index: number) => {
    // 点击事件防抖
    if (Date.now() - timeClickRecord < 100) {
