import { FC, forwardRef, Fragment, useDeferredValue, useEffect, useState } from "react";
...
const EditorWrapper = forwardRef<EditorRef, GlobalConfig>((config, ref) => {
  ...
  // 模拟 componentDidMount - 组件挂载后执行
  useEffect(() => {
    setMounted(true); // 标记页面已挂载
    return () => {}; // 清理函数 - 模拟 componentWillUnmount
  }, []); // 空依赖数组，只在挂载时执行一次
