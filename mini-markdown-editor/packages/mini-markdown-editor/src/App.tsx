...
import { N } from "vitest/dist/chunks/reporters.DTtkbAtP.js";
...
const App: FC = () => {
  ...
  // 编辑器内容改变
  const { qiankunProps, setQiankunProps } = useEditorContentStore();
  const handleChange = (val: string, view: ViewUpdate) => {
    // console.log(val, view);
    // 传入主应用
    (qiankunProps as any)?.onMessage({
      val,
      view,
    });
  };
  ...
  const { setIsOnlyPreview } = useToolbarStore();
  const handleSendMessage = (res: any) => {
    // 如果页面type是只读，直接切换为只读窗口
    if (res?.type === "look") {
      setIsOnlyPreview(true);
    } else if (res?.type === "edit") {
      setIsOnlyPreview(false);
    }
    if (res?.curerntDoc?.content) {
      setContent(res?.curerntDoc?.content);
    }
  };
  // 监听 qiankunProps 事件
  const handleQiankunProps = (props: any) => {
    setQiankunProps(props);
  };

  const handleQiankunMount = (isMount: boolean) => {
    if (!isMount) {
      console.log("卸载外部乾坤");
      eventBus.off("sendMessage", handleSendMessage);
      eventBus.off("qiankunProps", handleQiankunProps);
      eventBus.off("qiankunMount", handleQiankunMount);
    }
  };
  ...
  useEffect(() => {
    ...
    console.log("重新注册外部乾坤");
    eventBus.on("sendMessage", handleSendMessage);
    eventBus.on("qiankunProps", handleQiankunProps);
    // 监听外部乾坤卸载
    eventBus.on("qiankunMount", handleQiankunMount);
    return () => {
      handleQiankunMount(false);
    };
  }, []);
