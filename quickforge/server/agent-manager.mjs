...
// Sessions explicitly deleted must not be re-written by in-flight persist calls.
...
async function purgeSessionStorage(sessionId) {
  try {
    ...
    logger.error(`Failed to purge session storage ${sessionId}:`, err, { sessionId })
  }
}
...
function resetIdleTimer(session) {
  ...
  session.idleTimer = setTimeout(() => {
    ...
    logger.info(`Session ${session.sessionId} idle timeout (${IDLE_TIMEOUT_MS / 1000}s), evicting from memory...`, { sessionId: session.sessionId })
    evictAgent(session.sessionId).catch((err) =>
      logger.error(`Failed to evict idle agent ${session.sessionId}:`, err, { sessionId: session.sessionId }),
    )
  }, IDLE_TIMEOUT_MS)
}
...
export async function createAgent(sessionId, config = {}) {
  ...
  if (existing) {
    ...
    const nextTitle = typeof config.title === 'string' ? config.title.trim() : ''
...
export async function runPrompt(sessionId, message) {
  ...
  // Skip when the user has already renamed the session (titleGenerated).
...
/**
 * Remove an in-memory agent without deleting persisted session data.
 * Used for idle timeout and server shutdown.
 */
export async function evictAgent(sessionId) {
  const session = agentSessions.get(sessionId)
  if (!session) return

  logger.info(`Evicting session ${sessionId} from memory (status: ${session.status})`, { sessionId, status: session.status })
  ...
  try {
    ...
    logger.error(`Failed to persist session ${sessionId} before evict:`, err, { sessionId })
  }
...
/**
 * Destroy an agent session and permanently delete persisted data (user delete).
 */
export async function destroyAgent(sessionId) {
  ...
  await purgeSessionStorage(sessionId)
}
...
export async function restoreAgent(sessionId) {
  if (isSessionDeleted(sessionId)) {
    logger.warn(`Cannot restore deleted session ${sessionId}`, { sessionId })
    return null
  }
...
/**
 * Update session title (user rename). Marks title as user-set so AI auto-title
 * does not overwrite it, and persists to storage.
 */
export async function updateSessionTitle(sessionId, title) {
  ...
  if (session) {
    ...
    session.titleGenerated = true
    await persistSession(session)
    emitSessionEvent(session, { type: 'title_updated', title: normalized })
    agentEvents.emit('agent_event', { sessionId, type: 'title_updated', title: normalized })
  ...
  const now = new Date().toISOString()
  await writeSessionValue(sessionId, { ...sessionData, title: normalized, lastModified: now })
  await atomicUpdate('sessions-metadata', (data) => {
    if (data[sessionId]) {
      data[sessionId] = { ...data[sessionId], title: normalized, lastModified: now }
    }
    return data
  })
...
export async function shutdown() {
  ...
  await Promise.all(ids.map((id) => evictAgent(id)))
}
