...
function parseHourMinute(schedule) {
  const hour = Number(schedule?.hour)
  const minute = Number(schedule?.minute ?? 0)
  if (!Number.isInteger(hour) || hour < 0 || hour > 23) {
    throw requestError('hour must be between 0 and 23')
  }
  if (!Number.isInteger(minute) || minute < 0 || minute > 59) {
    throw requestError('minute must be between 0 and 59')
  }
  return { hour, minute }
}

function buildScheduleRule(frequency, { hour, minute, period, weekDay, monthDay }) {
  const timeLabel = `${pad(hour)}:${pad(minute)}`
  const periodLabel = SCHEDULE_PERIOD_LABELS[String(period || '').trim()] || ''
  if (frequency === 'weekly') {
    const weekLabel = SCHEDULE_WEEKDAY_LABELS[Number(weekDay)] || `周${weekDay}`
    return periodLabel ? `每${weekLabel} ${periodLabel} ${timeLabel}` : `每${weekLabel} ${timeLabel}`
  }
  if (frequency === 'monthly') {
    return periodLabel ? `每月 ${monthDay} 号 ${periodLabel} ${timeLabel}` : `每月 ${monthDay} 号 ${timeLabel}`
  }
  return periodLabel ? `每天 ${periodLabel} ${timeLabel}` : `每天 ${timeLabel}`
}

function parseStructuredSchedule(schedule) {
  const frequency = String(schedule?.frequency || 'daily').trim()
  const { hour, minute } = parseHourMinute(schedule)
  const period = String(schedule?.period || '').trim()
  ...
  if (frequency === 'weekly') {
    ...
    throw requestError('frequency must be daily, weekly, or monthly')
  }

  if (!cronRegex.test(cronExpression)) throw requestError('cronExpression is invalid')
  const nextRun = nextCronRun(cronExpression)
  if (!nextRun) throw requestError('Unable to calculate next cron run')

  return {
    cronExpression,
    scheduleRule: buildScheduleRule(frequency, { hour, minute, period, weekDay, monthDay }),
    nextRunAt: nextRun.toISOString(),
  }
}

function parseScheduledTaskInstruction(input) {
  const instruction = String(input?.instruction || '').trim()
  if (!instruction) return { needMoreInfo: true, question: '请输入 AI 将发送的内容。' }

  const schedule = input?.schedule
  if (!schedule || typeof schedule !== 'object') {
    return { needMoreInfo: true, question: '请选择执行时间。' }
  }

  try {
    const built = parseStructuredSchedule(schedule)
    const title = String(input?.title || '').trim() || extractTitle(instruction)
    return {
      needMoreInfo: false,
      task: {
        ...
        instruction,
        scheduleType: 'cron',
        scheduleRule: built.scheduleRule,
        cronExpression: built.cronExpression,
        nextRunAt: built.nextRunAt,
      },
    }
  } catch (error) {
    return { needMoreInfo: true, question: error?.message || '定时规则无效，请检查执行时间。' }
  }
}
...
export async function handleScheduledTasksApi(req, res, url) {
  ...
  if (req.method === 'POST' && url.pathname === '/api/scheduled-tasks/parse') {
    ...
    sendJson(res, 200, await parseScheduledTaskInstruction(body))
    return
  }
