...
import { openUrlInSystemBrowser } from '@/lib/uwork-parent-bridge'
...
export const ChatSidebar = memo(function ChatSidebar({
  ...
  const sectionHeaderClass = 'mb-1 flex w-full items-center gap-1 rounded-lg px-2 py-1 text-sm font-medium leading-5 text-muted-foreground/68 transition-colors hover:bg-muted/28 hover:text-foreground/82'
  ...
  const rowClass = `quickforge-sidebar-row group relative flex items-center gap-2 overflow-hidden rounded-lg px-2 py-1.5 text-left transition-all duration-160 ease-out hover:-translate-y-px active:translate-y-0 ${rowHoverShadowClass}`
  const activeRowClass = 'sidebar-row-selected text-foreground/95'
  const projectActiveRowClass = 'sidebar-row-active-project text-foreground/90'
  const inactiveRowClass = 'text-muted-foreground/72 hover:bg-primary/10 hover:text-foreground/90'
  const sessionInactiveRowClass = 'text-muted-foreground/76 hover:bg-primary/12 hover:text-foreground/92'
  ...
  const iconButtonClass = `size-7 shrink-0 rounded-full text-muted-foreground/55 transition-all duration-160 ease-out hover:-translate-y-px hover:bg-muted/30 hover:text-foreground/85 active:translate-y-0 ${iconHoverShadowClass}`
  const actionOverlayClass = 'quickforge-sidebar-actions pointer-events-none absolute inset-y-0 right-1 flex items-center gap-1 rounded-r-lg bg-background pl-3 opacity-0 transition-opacity duration-160'
  const actionOverlayVisibleClass = 'pointer-events-auto opacity-100'
  const overlayIconButtonClass = 'size-7 shrink-0 rounded-full bg-background/95 text-muted-foreground/65 shadow-[0_8px_20px_-16px_rgb(15_23_42_/_0.42)] ring-1 ring-border/70 transition-all duration-160 ease-out hover:-translate-y-px hover:bg-muted/35 hover:text-foreground/86 active:translate-y-0'
  const overlayDangerIconButtonClass = 'size-7 shrink-0 rounded-full bg-background/95 text-muted-foreground/65 shadow-[0_8px_20px_-16px_rgb(15_23_42_/_0.42)] ring-1 ring-border/70 transition-all duration-160 ease-out hover:-translate-y-px hover:bg-destructive/10 hover:text-destructive active:translate-y-0'
  ...
  useEffect(() => {
    ...
    document.addEventListener('mouseleave', clearHoveredRow)
    window.addEventListener('blur', clearHoveredRow)
    return () => {
      document.removeEventListener('mouseleave', clearHoveredRow)
      window.removeEventListener('blur', clearHoveredRow)
    }
  }, [])
    ...
    >
                    ...
                    <div className="space-y-0.5">
                      {projects.length === 0 ? (
                        ...
                        projects.map((item) => {
                          ...
                          const projectRowKey = `project-${item.id}`
                          const showProjectActions = hoveredRowKey === projectRowKey || menuOpen

                          return (
                            <div key={item.id}>
                              <div
                                className={cn(rowClass, active ? projectActiveRowClass : inactiveRowClass, menuOpen && 'z-20 overflow-visible')}
                                onMouseEnter={() => setHoveredRowKey(projectRowKey)}
                                onMouseLeave={() => setHoveredRowKey((current) => current === projectRowKey ? null : current)}
                              >
                                <button
                                  type="button"
                                ...
                                <div className={cn(actionOverlayClass, showProjectActions && actionOverlayVisibleClass)}>
                                    ...
                                    {projectSessions.length === 0 && !loaded ? (
                                      ...
                                      <>
                                        {projectSessions.map((session) => {
                                          ...
                                          const sessionRowKey = `session-${session.id}`
                                          const showSessionActions = hoveredRowKey === sessionRowKey
                                          return (
                                            <div
                                              key={session.id}
                                              className={cn(rowClass, 'gap-1', selected ? activeRowClass : sessionInactiveRowClass)}
                                              onMouseEnter={() => setHoveredRowKey(sessionRowKey)}
                                              onMouseLeave={() => setHoveredRowKey((current) => current === sessionRowKey ? null : current)}
                                            >
                                              ...
                                              <div className={cn(actionOverlayClass, showSessionActions && actionOverlayVisibleClass)}>
                                                <Button
                                                  variant="ghost"
                                                  size="icon"
                                                  className={overlayIconButtonClass}
                  ...
                  {globalSessions.length === 0 && !globalHasMore ? (
                    ...
                    <div className="space-y-0.5">
                      {globalSessions.map((session) => {
                        const selected = currentSessionId === session.id
                        const sessionRowKey = `global-session-${session.id}`
                        const showSessionActions = hoveredRowKey === sessionRowKey
                        return (
                          <div
                            key={session.id}
      ...
      <div className="mt-auto shrink-0 border-t border-border px-3 py-3">
        <div className={cn('flex gap-2', !sidebarOpen && 'flex-col')}>
          <button
            type="button"
            className={cn(rowClass, 'min-w-0 flex-1', inactiveRowClass)}
          ...
          <button
            type="button"
            className={cn(rowClass, 'min-w-0 flex-1', inactiveRowClass)}
            onClick={() => openUrlInSystemBrowser()}
          ...
          >
            ...
            {sidebarOpen ? <span className={sessionTitleClass}>{t('openInBrowser')}</span> : null}
          </button>
        </div>
      </div>
