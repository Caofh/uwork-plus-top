...
import { applyIframeEmbedClass, applyUworkParentBridge } from '@/lib/uwork-parent-bridge'
import App from './App.tsx'

patchThinkingSelector()
applyClipboardPolyfill()
applyIframeEmbedClass()
applyUworkParentBridge()

if (import.meta.env.PROD && 'serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('/sw.js').catch(() => {
      // Service Worker registration should not block the app.
      console.log('Service Worker registration should not block the app')
    })
  })
}
