...
export class ServerAgent {
  ...
  static async destroyOnServer(sessionId: string, baseUrl = ''): Promise<void> {
    const res = await fetch(`${baseUrl}/api/agents/${encodeURIComponent(sessionId)}`, {
      method: 'DELETE',
    })
    if (!res.ok) {
      const payload = await res.json().catch(() => null) as { error?: string } | null
      throw new Error(payload?.error || `Failed to destroy agent: HTTP ${res.status}`)
    }
  }

  static async updateTitleOnServer(sessionId: string, title: string, baseUrl = ''): Promise<void> {
    const res = await fetch(`${baseUrl}/api/agents/${encodeURIComponent(sessionId)}/title`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
    ...
    if (!res.ok) {
      const payload = await res.json().catch(() => null) as { error?: string } | null
      throw new Error(payload?.error || `Failed to update session title: HTTP ${res.status}`)
    }
  }
