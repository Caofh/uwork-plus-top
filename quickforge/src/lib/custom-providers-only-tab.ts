...
const OLLAMA_LOCAL_MODEL_GUIDE_URL =
  (import.meta.env.VITE_DOC_OLLAMA_GUIDE as string | undefined)?.trim() ?? ''
  ...
  override render(): TemplateResult {
    return html`
      <div class="flex flex-col gap-6">
        ${OLLAMA_LOCAL_MODEL_GUIDE_URL
          ? html`
              <a
                class="w-fit text-sm text-primary hover:underline"
                href=${OLLAMA_LOCAL_MODEL_GUIDE_URL}
                target="_blank"
                rel="noopener noreferrer"
              >
                ${t('ollamaLocalModelGuide')}
              </a>
            `
          : ''}
